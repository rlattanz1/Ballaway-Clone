class Api::CartItemsController < ApplicationController
    ...
    def create
        ...
        quantity = cart_items_params[:quantity]
        @cart_item = CartItem.find_by(user_id: user_id, product_id: product_id, quantity: quantity)
        if @cart_item
    ...
    def destroy
        @cart_item = CartItem.find_by(id: params[:id])
        if @cart_item && @cart_item.user_id == current_user.id
            @cart_item.destroy
            render :show
        else
